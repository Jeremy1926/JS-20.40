...
AFortPickupAthena* SpawnPickupNew(ABuildingContainer* Container, FFortItemEntry Entry, AFortPlayerPawn* Pawn = nullptr, int OverrideCount = -1)
{
	if (!&Entry)
		return nullptr;
...
bool Looting::SpawnLootHook(ABuildingContainer* Container)
{
	...
	for (auto& LootDrop : ChooseLootForContainer(RealTierGroup)) SpawnPickupNew(Container, LootDrop);
...
void Looting::Hook()
{
	Utils::Hook(Jeremy::ImageBase + 0x6820DC4, SpawnLootHook);
