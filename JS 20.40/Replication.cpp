...
{
	return (decltype(&GetViewTarget)(Jeremy::ImageBase + Jeremy::Offsets::GetViewTarget))(Controller);
}
...
{
	return *(TSet<FNetworkGUID>*)(__int64(Conn) + 0x1488); //maybe check?
}
...
{
	return *(TSet<FName>*)(__int64(Conn) + 0x1528); //maybe check?
}
...
{
	void (*CallPreReplication)(AActor*, UNetDriver*) = decltype(CallPreReplication)(Jeremy::ImageBase + Jeremy::Offsets::CallPreReplication);
	void (*RemoveNetworkActor)(UNetDriver*, AActor*) = decltype(RemoveNetworkActor)(Jeremy::ImageBase + Jeremy::Offsets::RemoveNetworkActor);
	...
	for (auto& ActorInfo : ActiveNetworkObjects)
	{
		...
		if (!ActorInfo->bPendingNetUpdate)
		{
			...
			float ServerTickTime = 1.f / clamp(1.f / DeltaTime, 1.f, Options::iMaxTickRate);
...
{
	bool (*IsNetRelevantFor)(const AActor*, const AActor*, const AActor*, const FVector&) = decltype(IsNetRelevantFor)(Actor->VTable[0x99]);
...
{
	bool (*IsRelevancyOwnerFor)(const AActor*, const AActor*, const AActor*, const AActor*) = decltype(IsRelevancyOwnerFor)(Actor->VTable[0x99 + 2]);
	AActor* (*GetNetOwner)(const AActor*) = decltype(GetNetOwner)(Actor->VTable[0x9F]);
...
{
	bool (*ClientHasInitializedLevelFor)(const UNetConnection*, const AActor*) = decltype(ClientHasInitializedLevelFor)(Jeremy::ImageBase + Jeremy::Offsets::ClientHasInitializedLevelFor);
...
__forceinline void Replication::SendClientAdjustment(APlayerController* PlayerController)
{
	...
	if (Interface)
	{
		auto SendClientAdjustmemt = (void (*)(INetworkPredictionInterface*)) (Jeremy::ImageBase + Jeremy::Offsets::SendClientAdjustment);
...
__forceinline void Replication::PrioritizeActors(UNetDriver* Driver, UNetConnection* Conn, TArray<FNetViewer>& ConnViewers) {
	void (*CloseActorChannel)(UActorChannel*, uint8) = decltype(CloseActorChannel)(Jeremy::ImageBase + Jeremy::Offsets::CloseActorChannel);
	UActorChannel* (*FindActorChannelRef)(UNetConnection*, const TWeakObjectPtr<AActor>&) = decltype(FindActorChannelRef)(Jeremy::ImageBase + Jeremy::Offsets::FindActorChannelRef);
	void (*StartBecomingDormant)(UActorChannel*) = decltype(StartBecomingDormant)(Jeremy::ImageBase + Jeremy::Offsets::StartBecomingDormant);

	GetNetTag(Driver)++;
	for (auto& Temp : Conn->SentTemporaries)
		Temp->NetTag = GetNetTag(Driver);

	int32 MaxSortedActors = ConsiderList.Num() + GetDestroyedStartupOrDormantActors(Driver).Num(); //checked
	if (MaxSortedActors > 0) {
		...
		for (auto& ActorInfo : ConsiderList) {
			...
			//cheked IsActorRelevantToConnection
			...
			if (Actor->bOnlyRelevantToOwner)
			{
				...
				auto PriorityConnection = IsActorOwnedByAndRelevantToConnection(Actor, ConnViewers, bHasNullViewTarget); //checked
...
__forceinline bool Replication::IsNetReady(UNetConnection* Connection, bool bSaturate) {
	bool (*IsNetReady)(UNetConnection*, bool) = decltype(IsNetReady)(Jeremy::ImageBase + Jeremy::Offsets::IsNetReady);
...
__forceinline size_t Replication::ProcessPrioritizedActors(UNetDriver* Driver, UNetConnection* Conn, TArray<FNetViewer>& ConnViewers) {
	UActorChannel* (*CreateChannelByName)(UNetConnection*, FName*, EChannelCreateFlags, int32_t) = decltype(CreateChannelByName)(Jeremy::ImageBase + Jeremy::Offsets::CreateChannelByName);
	__int64 (*SetChannelActor)(UActorChannel*, AActor*) = decltype(SetChannelActor)(Jeremy::ImageBase + Jeremy::Offsets::SetChannelActor);
	__int64 (*ReplicateActor)(UActorChannel*) = decltype(ReplicateActor)(Jeremy::ImageBase + Jeremy::Offsets::ReplicateActor);
	void (*CloseActorChannel)(UActorChannel*, uint8) = decltype(CloseActorChannel)(Jeremy::ImageBase + Jeremy::Offsets::CloseActorChannel);
	int64(*SetChannelActorForDestroy)(UActorChannel*, FActorDestructionInfo*) = decltype(SetChannelActorForDestroy)(Jeremy::ImageBase + Jeremy::Offsets::SetChannelActorForDestroy);
	bool (*SupportsObject)(void*, const UObject*, const TWeakObjectPtr<UObject>*) = decltype(SupportsObject)(Jeremy::ImageBase + Jeremy::Offsets::SupportsObject);
	UObject* (*GetArchetype)(UObject*) = decltype(GetArchetype)(Jeremy::ImageBase + Jeremy::Offsets::GetArchetype);

	if (!IsNetReady(Conn, false)) return 0;

	static auto ActorFName = UKismetStringLibrary::Conv_StringToName(L"Actor");
	int i = 0;
			...
			if (bRecentlyRelevant) {
				if (!Channel && SupportsObject(GetGuidCache(Driver).Get(), Actor->Class, nullptr) && SupportsObject(GetGuidCache(Driver).Get(), Actor->bNetStartup ? Actor : GetArchetype(Actor), nullptr)) {
					if (bLevelInitializedForActor) {
						...
						ActorInfo->NextUpdateTime = (float)UGameplayStatics::GetTimeSeconds(UWorld::GetWorld()) + 0.2f * FRand();
				}
				if (Channel) {
					if (bRelevant)
...
void Replication::ServerReplicateActors(UNetDriver* Driver, float DeltaTime)
{
	...
	void (*CloseConnection)(UNetConnection*) = decltype(CloseConnection)(Jeremy::ImageBase + Jeremy::Offsets::CloseConnection);
